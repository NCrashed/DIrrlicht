...
module irrlicht.video.IMaterialRenderer;
...
interface IMaterialRenderer
{
	...
	* Params:
	* 	material=  The new material parameters to be set. The renderer
	...
	* 	lastMaterial=  The material parameters which have been set before
	* this material.
	* 	resetAllRenderstates=  True if all renderstates should really be
	...
	* fogenable.
	* 	services=  Interface providing some methods for changing
	...
	* Params:
	* 	service=  Pointer to interface providing methos for setting
	* constants and other things.
	* 	vtxtype=  Vertex type with which the next rendering will be done.
	...
	* Returns: Returns true if everything is ok, and false if nothing should
	...
	* function.
	* Returns: Returns 0 if everything is running fine. Any other value is material renderer
